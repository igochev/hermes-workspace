...
function assertActionAllowed(
  ...
  if (action === 'cancel') {
    if (workItem.status === 'done' || workItem.status === 'cancelled') {
      throw new Error(
        'cancel is only valid for non-terminal work items (inbox, ready, active, blocked)',
      )
    }
    return
  }
  ...
  if (action === 'back_to_research') {
    if (
      (workItem.status === 'active' &&
        (workItem.phase === 'build' || workItem.phase === 'review')) ||
      (workItem.status === 'blocked' && workItem.phase === 'build')
    ) {
      return
    }
    throw new Error(
      'back_to_research is only valid for active build, active review, or blocked build work items',
    )
  }
  ...
  if (action === 'back_to_inbox') {
    ...
    throw new Error(
      'back_to_inbox is only valid for active research work items',
    )
  }
  ...
  if (action === 'send_to_planning') {
    ...
    throw new Error(
      'send_to_planning is only valid for inbox research work items',
    )
  }
  ...
  if (action === 'request_deploy_approval') {
    if (workItem.status === 'active' && workItem.phase === 'deploy') return
    throw new Error(
      'request_deploy_approval is only valid for active deploy work items',
    )
  }
...
export function applyWorkItemLifecycleTransition(
  ...
  if (input.action === 'send_to_planning') {
    ...
    const withHistory = appendWorkItemHistoryEntry(updated.id, {
      ...
      phase: 'research',
      note: notes
        ? `Sent to planning with Researcher: ${notes}`
        : 'Sent to planning with Researcher.',
    ...
    if (!withHistory)
      throw new Error('Failed to record lifecycle history entry')
  ...
  if (input.action === 'mark_ready') {
    ...
    const withHistory = appendWorkItemHistoryEntry(updated.id, {
      ...
      phase: undefined,
      note: notes
        ? `Planning complete; marked ready for build: ${notes}`
        : 'Planning complete; marked ready for build.',
      missionId: updated.missionId,
      sessionKey: updated.sessionKeys.at(-1),
      sessionKeyPrefix: updated.missionSessionKeyPrefix,
      profile: updated.assignedProfile,
  ...
  if (input.action === 'request_deploy_approval') {
    ...
    if (!refreshed)
      throw new Error('Work item not found after lifecycle transition')
    return {
      workItem: refreshed,
      ...(approval ? { approval } : {}),
    }
  ...
  if (input.action === 'cancel') {
    if (!notes?.trim()) {
      throw new Error(
        'cancel requires a reason note explaining why the work item is being cancelled',
      )
    }
    const updated = updateWorkItem(workItem.id, {
      status: 'cancelled',
  ...
  if (input.action === 'back_to_research') {
    ...
    const withHistory = appendWorkItemHistoryEntry(updated.id, {
      ...
      phase: 'research',
      note: notes
        ? `Returned to research for additional planning: ${notes}`
        : 'Returned to research for additional planning.',
      missionId: updated.missionId,
      sessionKey: updated.sessionKeys.at(-1),
      sessionKeyPrefix: updated.missionSessionKeyPrefix,
      profile: updated.assignedProfile,
  ...
  if (input.action === 'back_to_build') {
    ...
    const withHistory = appendWorkItemHistoryEntry(updated.id, {
      ...
      phase: 'build',
      note: notes
        ? `Returned to build for fixes: ${notes}`
        : 'Returned to build for fixes.',
      missionId: updated.missionId,
      sessionKey: updated.sessionKeys.at(-1),
      sessionKeyPrefix: updated.missionSessionKeyPrefix,
      profile: updated.assignedProfile,
  ...
  if (input.action === 'back_to_inbox') {
    ...
    const withHistory = appendWorkItemHistoryEntry(updated.id, {
      ...
      phase: 'research',
      note: notes
        ? `Returned to inbox for refinement: ${notes}`
        : 'Returned to inbox for refinement.',
      missionId: updated.missionId,
      sessionKey: updated.sessionKeys.at(-1),
      sessionKeyPrefix: updated.missionSessionKeyPrefix,
      profile: updated.assignedProfile,
  ...
  updated = appendWorkItemHistoryEntry(updated.id, {
    ...
    phase: 'review',
    note: notes
      ? `Requested review; moved work item into review: ${notes}`
      : 'Requested review; moved work item into review.',
    missionId: updated.missionId,
    sessionKey: updated.sessionKeys.at(-1),
    sessionKeyPrefix: updated.missionSessionKeyPrefix,
    profile: updated.assignedProfile,
